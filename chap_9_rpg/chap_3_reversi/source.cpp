...
bool CheckCanPlace(
    int _color,             // 石の色
    VEC2 _position,         // 座標
    bool _turnOver = false) // ひっくり返すかどうか
{
...
void DrawScreen()
{
    ...
    else
    {
        ...
        // [6-5-21]勝者を判定する
        if (blackCount > whiteCount)        // [6-5-22]黒のほうが多ければ
        {
            winner = TURN_BLACK;            // [6-5-23]黒の勝ち
        }
        else if (whiteCount > blackCount)   // [6-5-24]白のほうが多ければ
        {
    ...
    while (1)
    {
        ...
        default:// [6-6-15]その他のキーが押されたら
            ...
            case MODE_1P:       // [6-6-17]AIと対戦するモードなら
                ...
                isPlayer[TURN_BLACK] = true;    // [6-6-18]黒をプレイヤーにする
                isPlayer[TURN_WHITE] = false;   // [6-6-19]白をプレイヤーにしない

                break;

            case MODE_2P:       // [6-6-20]人間どうしの対戦モードなら
            ...
            case MODE_WATCH:    // [6-6-22]AIどうし対決の観戦モードなら
...
start:  // [6-9-2]開始ラベル
    ...
    while (1)
    {
        ...
        if (!CheckCanPlaceAll(turn))
        {
            ...
            if (!CheckCanPlaceAll(turn))
            {
                turn = TURN_NONE;// [6-9-10]決着が付いたことにする
